...
def extract_weight_to_half(weight: torch.Tensor, scale_list: torch.Tensor,
    ...
    if source_bit_width == 8:
        ...
        func = (
            kernels.int4WeightExtractionHalf if scale_list.dtype == torch.half
    ...
    with torch.cuda.device(weight.device):
        ...
        out = torch.empty(
            n,
            m * (8 // source_bit_width),
            dtype=scale_list.dtype,
            device='cuda')
        stream = torch.cuda.current_stream()
...
def quantize(model, weight_bit_width, empty_init=False, device=None):
    ...
    for layer in model.layers:
        ...
        layer.self_attention.dense = QuantizedLinear(
            ...
            device=layer.self_attention.dense.weight.device
            if device is None else device,
            empty_init=empty_init)
