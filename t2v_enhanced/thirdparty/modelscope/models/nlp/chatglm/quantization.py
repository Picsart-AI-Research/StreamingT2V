...
def compress_int4_weight(weight: torch.Tensor):  # (n, m)
    with torch.cuda.device(weight.device):
        ...
        if kernels is None:
            raise RuntimeError(
                'kernels is None, please check whether it is correctly initialized.'
            )
    ...
    def __init__(self,
        ...
        if weight_tensor is None or empty_init:
            ...
            self.weight = torch.round(
                weight_tensor / self.weight_scale[:, None]).to(torch.int8)
        ...
        if bias_tensor is not None:
            self.bias = Parameter(
                bias_tensor.to(kwargs['device']), requires_grad=False)
        else:
            self.bias = None
...
def quantize(model, weight_bit_width, empty_init=False, **kwargs):
    ...
    for layer in model.layers:
        ...
        layer.attention.dense = QuantizedLinear(
            weight_bit_width=weight_bit_width,
            weight_tensor=layer.attention.dense.weight.to(
                torch.cuda.current_device()),
