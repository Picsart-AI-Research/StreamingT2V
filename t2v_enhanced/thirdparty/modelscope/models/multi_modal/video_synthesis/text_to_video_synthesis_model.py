...
def _i(tensor, t, x):
    ...
    return tensor[t.to(tensor.device)].view(shape).to(x.device).to(x.dtype)


def diffuse(diff_model, x0, t, noise=None):
    noise = torch.randn_like(x0) if noise is None else noise
    alphas = _i(diff_model.alphas_cumprod, t, x0)
    xt = torch.sqrt(alphas) * x0 + torch.sqrt(1 - alphas) * noise
    return xt
    ...
    def forward(self, input: Dict[str, Any]):
        ...
        with torch.no_grad():
            ...
            for vd_data in video_data_list:
                encoder_posterior = self.autoencoder.encode(vd_data.to(self.autoencoder.encoder.conv_in.weight.device))
            ...
            video_data_feature = rearrange(
                video_data_feature, '(b f) c h w -> b c f h w', b=batch_size).to(self.device)
        ...
        with torch.no_grad():
            ...
            with amp.autocast(enabled=True):
                ...
                noised_lr = diffuse(self.diffusion, video_data_feature, t, noise)
                x0 = self.diffusion.ddim_sample_loop(
                    noise=noised_lr.to(self.device),  # shape: b c f h w
                    model=self.sd_model,
                    model_kwargs=[{
                        'y':
