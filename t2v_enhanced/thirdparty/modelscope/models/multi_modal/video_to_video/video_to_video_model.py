...
from .temporal_transformer_hook import (
    TemporalAttentionHookManager,
    TemporalAttentionHookMode
)
...
class VideoToVideo(TorchModel):
    ...
    def __init__(self, model_dir, *args, **kwargs):
        ...
        self.forward_hook_manager = TemporalAttentionHookManager()
        logger.info(f"Registering forward hook for:")
        for name, module in self.generator.named_modules():
            if isinstance(module, TemporalTransformer):
                for i, t_block in enumerate(module.transformer_blocks):
                    for j, layer in enumerate([t_block.attn1, t_block.attn2]):
                        if not isinstance(layer, MemoryEfficientCrossAttention):
                            logger.error(f"{layer} is not of type {MemoryEfficientCrossAttention}")
                            raise RuntimeError()
                        layername = name + f'.{i}.attn{j + 1}'
                        layer.register_forward_hook(
                            self.forward_hook_manager.temp_att_read_write_hook(layername)
                        )
                        logger.info(f"\t\t'{layername}' has now {len(layer._forward_hooks)} hooks")
    ...
    def forward(self, input: Dict[str, Any]):
        ...
        negative_y = self.clip_encoder(negative_prompt).detach()
        self.negative_y = negative_y
        ...
        logger.info(f"Video size is {frames_num}")
        ...
        with amp.autocast(enabled=True):
            ...
            logger.info(f"Total noise levels: {total_noise_levels}")
            ...
            # process first chunk
            noise_all = torch.randn_like(video_data_feature)
            noised_latents_all = self.diffusion.diffuse(video_data_feature, t, noise_all)

            model_kwargs = [{'y': y}, {'y': self.negative_y}]

            generated_video_latents = self.diffusion.sample(
                noise=noised_latents_all,
            ...
            vid_tensor_feature = 1. / scale_factor * generated_video_latents
            ...
            decode_data = []
            for vd_data in vid_tensor_feature:
                tmp = self.autoencoder.decode(vd_data[None])
                decode_data.append(tmp)

        gen_video = rearrange(
            torch.cat(decode_data),
            '(b f) c h w -> b c f h w',
            b=cfg.batch_size
        )
