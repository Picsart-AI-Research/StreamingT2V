...
class GaussianDiffusion_SDEdit(object):
    ...
    def sample(self,
        ...
        overlap_size = kwargs.pop('overlap_size')

        logging.info(f"Chunk Size: {chunk_size}")
        def model_fn(xt, sigma, i):
            # denoising
            ...
            while chunk_start + chunk_size <= x0.shape[2]:
                xt_chunk = xt[:, :, chunk_start:chunk_start + chunk_size]
                ...
                x0[:, :, chunk_start + random_offset:chunk_start + chunk_size] = x0_chunk[:, :, random_offset:]
                chunk_start += chunk_size - overlap_size

            if chunk_start + overlap_size > x0.shape[2]:

                raise NotImplementedError(f"Video of size={x0.shape[2]} is not dividable into chunks "
        ...
        return (x0, intermediates) if return_intermediate else x0
